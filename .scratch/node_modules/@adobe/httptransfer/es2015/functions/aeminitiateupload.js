    ...
    return _wrapAsyncGenerator(function* () {
      ...
      try {
        for (var _iterator = _asyncIterator(transferAssets), _step; _iteratorAbruptCompletion = !(_step = yield _awaitAsyncGenerator(_iterator.next())).done; _iteratorAbruptCompletion = false) {
          ...
          {
            controller.notify(TransferEvents.AEM_INITIATE_UPLOAD, _this.name, transferAsset);
      ...
      try {
        ...
        if (typeof window !== 'undefined' && typeof window.document !== 'undefined') {
          ...
          headers['csrf-token'] = yield _awaitAsyncGenerator(getCSRFToken(origin, _this.options, assets[0].target.headers));
        }
        const initiateResponse = yield _awaitAsyncGenerator(retry( /*#__PURE__*/_asyncToGenerator(function* () {
          return postForm(`${folderUrl}.initiateUpload.json`, form, _objectSpread({
        ...
        for (let i = 0; i < assets.length; ++i) {
          ...
          if (!transferAsset.metadata.contentType) {
            ...
            transferAsset.metadata = new AssetMetadata(transferAsset.metadata.filename, contentType, transferAsset.metadata.contentLength);
          }
          transferAsset.multipartTarget = new AssetMultipart(uploadURIs, minPartSize, maxPartSize, transferAsset.target.multipartHeaders, completeUrl, uploadToken);
        }
        for (const transferAsset of assets) {
          controller.notify(TransferEvents.AFTER_AEM_INITIATE_UPLOAD, _this.name, transferAsset);
          yield transferAsset;
        }
      } catch (error) {
        for (const transferAsset of assets) {
