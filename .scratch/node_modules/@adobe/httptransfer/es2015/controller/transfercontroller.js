...
class TransferController extends EventEmitter {
  ...
  notify(eventName, functionName, transferItem, props) {
    const {
      transferAsset,
      transferPart
    } = splitTransferItem(transferItem);
    this.emit(eventName, new TransferEvent(functionName, undefined, transferAsset, transferPart, props));
  }
  ...
  notifyError(functionName, error, transferItem, props) {
    ...
    const event = new TransferEvent(functionName, error, transferAsset, transferPart, Object.assign({}, props, {
      firstError
    }));
  ...
  cleanupFailedTransfers() {
    ...
    return _asyncToGenerator(function* () {
      _this.failedAssets.forEach((transferEvent, transferItem) => {
        const {
          transferAsset
        } = splitTransferItem(transferItem);
        if (transferAsset.target.url.pathname) {
          logger.warn(JSON.stringify({
            "cleanup_failed_download": {
              transferItem,
              errors: _this.failedAssets[transferAsset]
            }
          }));
          try {
