 ...
 */
class UploadError extends BlockTransferError {
  /**
  ...
  static fromError(error, errorMessage = '') {
    ...
    if (message && code) {
      return new UploadError(BlockTransferError.getFullMessage(errorMessage, message), code, stack);
    }
    if (message) {
      return new UploadError(BlockTransferError.getFullMessage(errorMessage, message), errorCodes.UNKNOWN, stack);
    }
    if (typeof error === 'string') {
      return new UploadError(BlockTransferError.getFullMessage(errorMessage, error), errorCodes.UNKNOWN);
    }
    try {
      return new UploadError(BlockTransferError.getFullMessage(errorMessage, JSON.stringify(error)), errorCodes.UNKNOWN, stack);
      // eslint-disable-next-line no-unused-vars
    } catch (e) {
      return new UploadError(BlockTransferError.getFullMessage(errorMessage, error), errorCodes.UNKNOWN, stack);
    }
  }
