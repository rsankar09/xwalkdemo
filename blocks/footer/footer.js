import { decorateBlock, decorateIcons, loadBlock } from '../../scripts/aem.js';
...
export default async function decorate(block) {
  ...
  /*
   * The bundled markup is injected after decorateBlocks() has already run over
   * <main>, so nested blocks in it are never picked up: their CSS is never
   * requested and their decorator never runs. The subscribe form in the aside
   * column is the only one, and without this it renders as four bare
   * paragraphs. Loading it here is the same two calls decorateBlocks/loadBlocks
   * would have made.
   */
  await Promise.all(
    [...block.querySelectorAll('div.email-subscribe')].map((nested) => {
  ...
  /*
   * The bundled markup carries <span class="icon icon-*"> placeholders for the
   * social and privacy-choices glyphs. Nothing else swaps those for the real
   * SVGs, so without this the lists render as empty bullets.
   */
